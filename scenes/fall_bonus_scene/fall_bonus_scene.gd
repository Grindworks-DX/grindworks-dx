...
func animate_toon() -> void:
	var toon := %Toon
	toon.construct_toon(Util.get_player().toon.toon_dna)
	toon.set_animation('jump')
	toon.set_emotion(Toon.Emotion.SAD)
	toon.body.animator.speed_scale = -0.005
	toon.body.animator.seek(0.68)
	toon.body.animator.play()
	toon.legs.animator.speed_scale = -0.005
	toon.legs.animator.seek(0.68)
	toon.legs.animator.play()
	while true:
		await Task.delay(0.9)
		toon.body.animator.speed_scale = 0.005
		toon.body.animator.play()
		toon.legs.animator.speed_scale = 0.005
		toon.legs.animator.play()
